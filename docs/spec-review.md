...
projeto pausado está pausado *em alguma etapa*; se `paused` for uma etapa, a
...
- `content_items.status` é a **posição no pipeline** e é *derivado* da versão
...
o `files` guarda a metadata e a autorização acontece *antes* da assinatura,
...
no servidor *depois* do upload (o cliente mente antes); `Content-Disposition:
